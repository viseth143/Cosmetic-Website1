...
// ─── Protected E-Commerce Routes (Custom Middleware Applied) ──
// We use 'customer.auth' here to check your custom session_id
...
Route::delete('/admin/order/{id}/delete', function ($id) {
    \App\Models\Order::where('order_id', $id)->delete();
    return redirect()->route('admin.orders')->with('success', '🗑️ Order #' . $id . ' has been permanently deleted.');
})->name('admin.order.delete');

Route::get('/admin/users', [AdminStaffController::class, 'index'])->name('admin.users');
Route::post('/admin/users/make-admin/{id}', [AdminStaffController::class, 'makeAdmin'])->name('admin.users.makeAdmin');
Route::delete('/admin/users/remove-admin/{id}', [AdminStaffController::class, 'removeAdmin'])->name('admin.users.removeAdmin');
